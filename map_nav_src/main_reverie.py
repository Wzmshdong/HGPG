...
def build_dataset(args, rank=0):
    ...
    if args.aug is not None:
        aug_instr_data = construct_instrs(
            args.anno_pretrain_dir, args.dataset, [args.aug], 
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    for idx in range(start_iter, args.iters, args.log_every):
        ...
        if default_gpu:
            ...
            pl_loss = sum(listner.logs['PL_loss']) / max(len(listner.logs['PL_loss']), 1)
            entropy = sum(listner.logs['entropy']) / total
            writer.add_scalar("loss/critic", critic_loss, idx)
            writer.add_scalar("policy_entropy", entropy, idx)
            writer.add_scalar("loss/OG_loss", OG_loss, idx)
            writer.add_scalar("loss/IL_loss", IL_loss, idx)
            writer.add_scalar("loss/pl_loss", pl_loss, idx)
            writer.add_scalar("total_actions", total, idx)
            writer.add_scalar("max_length", length, idx)
            write_to_record_file(
                "\ntotal_actions %d, max_length %d, entropy %.4f, IL_loss %.4f, OG_loss %.4f, pl_loss %.4f, policy_loss %.4f, critic_loss %.4f" % (
                    total, length, entropy, IL_loss, OG_loss, pl_loss, policy_loss, critic_loss),
                record_file
            )
            # write_to_record_file(
            #     "\ntotal_actions %d, max_length %d, entropy %.4f, IL_loss %.4f, OG_loss %.4f, critic_loss %.4f" % (
            #         total, length, entropy, IL_loss, OG_loss, critic_loss),
            #     record_file
            # )
