...
class BaseAgent(object):
    ...
    def get_results(self, detailed_output=False):
        ...
        for k, v in self.results.items():
            ...
            output.append({'trajectory': v['path'], 'instr_id': k, 'pred_objid': v['pred_objid']})
            if detailed_output:
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0):
        ...
        params = sum(param.numel() for param in self.vln_bert.parameters())
        params_t = sum(
            p.numel() for p in self.vln_bert.parameters() if p.requires_grad
        )
        ...
        if self.args.optim_select is True:
            self.vln_bert_optimizer, self.sheduler = self.configure_optimizers()
        else:
        ...
        if self.args.progress:
            self.progress = nn.MSELoss(reduce=True, reduction='sum')
    ...
    #)
        for iter in range(1, n_iters + 1):
            ...
            # for k,v in self.vln_bert.named_parameters():
            #         if k == 'vln_bert.img_embeddings.pgn_module.tl_vectors':
            #             print(v)
            #             break
    ...
    def configure_optimizers(self):
        ...
        if self.args.optim == 'adamW':
            optimizer = torch.optim.AdamW(
                filter(lambda p: p.requires_grad, self.vln_bert.parameters()),
                lr=lr
            )
        else:
            optimizer = torch.optim.SGD(
