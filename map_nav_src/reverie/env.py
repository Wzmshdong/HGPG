...
class EnvBatch(object):
    ...
    def __init__(self, connectivity_dir, scan_data_dir=None, feat_db=None, batch_size=100):
        ...
        for i in range(batch_size):
            ...
            sim.setCameraVFOV(math.radians(self.vfov))#相机垂直视野角度
            sim.setBatchSize(1)
            sim.initialize()
    ...
    def get_cand_fit(self, scan, vp):
        fit = self.feat_db.get_image_feature1(scan, vp)
...
class ReverieObjectNavBatch(object):
    ...
    def __init__(
        ...
        self.gt_trajs = self._get_gt_trajs(self.data) # for evaluation/获得真实路径
        ...
        self.ix = 0
        self._load_nav_graphs() #加载每个scans的图，最短路径 最短距离
    ...
    def _next_minibatch(self, batch_size=None, **kwargs):
        ...
        if self.multi_endpoints:
            for i, item in enumerate(batch):#随机选择一个终点
    ...
    def _get_obs(self):#获得当前状态，候选节点，以及当前位置到最终位置的距离
        obs = []
